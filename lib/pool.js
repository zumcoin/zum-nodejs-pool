...
var cnHashing = require('zumcoin-multi-hashing');
...
if (!cnHashing || !cnHashing[cnAlgorithm]) {
    ...
    cryptoNightLite = cnHashing["cryptonightlite"];
...
function jobRefresh(loop, callback){
    ...
    getBlockTemplate(function(error, result){
            buffer = new Buffer(result.blocktemplate_blob, 'hex');
            new_hash = new Buffer(32);
        ...
        if (!currentBlockTemplate || result.height > currentBlockTemplate.height) {
            log('info', logSystem, 'New block to mine at height %d w/ difficulty of %d for version %s', [result.height, result.difficulty,result.blocktemplate_blob[0]]);
...
function processShare(miner, job, blockTemplate, params){
	...
	log('warn', logSystem, 'Expected: %s, received: %s', [hash.toString('hex'),  resultHash]);
