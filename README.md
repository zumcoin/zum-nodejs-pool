...
High performance Node.js (with native C addons) mining pool for CryptoNote based coins. Comes with lightweight example front-end script which uses the pool's AJAX API. Support for Cryptonight (Original, Monero v7, Stellite v7), Cryptonight Light (Original, Aeon v7, IPBC), Cryptonight Heavy (Sumokoin) and cryptonight-zum-lite (ZumCoin) algorithms.
...
```bash
git clone https://github.com/zumcoin/zum-nodejs-pool.git pool
cd pool
...
/* Set daemon type. Supported values: default, forknote (Fix block height + 1), bytecoin (ByteCoin Wallet RPC API, w/ block height fix), zumcoin (ByteCoin Wallet RPC API but no block height fix required)  */
"daemonType": "default",
...
"payments": {
    ...
    "minPayment": 1000000000, // 10 ZUM Miner balance required before sending payment
...
"blockUnlocker": {
    ...
    "depth": 60,
    "poolFee": 0.5, // 0.5% pool fee
    "devDonation": 0.0, // 0.0% donation to send to pool dev
...
Use this [example](https://github.com/zumcoin/zum-nodejs-pool/blob/master/deployment/cryptonote-nodejs-pool.service) to create the systemd service `/lib/systemd/system/cryptonote-nodejs-pool.service`
